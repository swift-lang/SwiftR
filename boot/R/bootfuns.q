...
boot.orig <- function(data, statistic, R, sim="ordinary", stype="i",
...
boot <- function(data, statistic, R, sim="ordinary", stype="i",
                 strata = rep(1, n), L=NULL, m=0, weights = NULL,
		 ran.gen=function(d, p) d, mle=NULL, simple=FALSE, swift=FALSE, ...)
{
#
# R replicates of bootstrap applied to  statistic(data)
# Possible sim values are: "ordinary", "balanced", "antithetic",
        ...
        if(stype == "f") {
            ...
            for(r in 1L:sum(R)) {
             ...
             if(!swift) {
                 ...
                 # Test code to call Swift: supports Mike Neale's example:
                 alists <- list()
                 for (r in 1L:sum(R)) alists[[r]] <- list(data, i[r, ], ...)
                 reslist <- swiftapply(statistic, alists)
...
#    glm.f <- formula(glmfit)
    ...
    out <- list(call=call,K=K,delta=c(CV, CV + cost.0), seed=seed)
    out
}
...
#  absolute value of the determinant of a matrix.
        if (any(is.na(mat)))
            ...
            out <- Inf
        else	out <- abs(prod(eigen(mat,only.values=TRUE)$values))
        out
    }
...
lines.saddle.distn <- function(x, dens=TRUE, h=function(u) u,
